...
if (@ARGV != 4) {
  print STDERR "Usage: $0 <path-to-commonvoice-corpus> <csvs_location> <dataset> <valid-train|valid-dev|valid-test>\n";
  print STDERR "e.g. $0 /export/data/cv_corpus_v1 /export/data/cv_corpus_v1/csv cv-valid-train valid-train\n";
  exit(1);
}

($db_base, $db_partition, $dataset, $out_dir) = @ARGV;
mkdir data unless -d data;
mkdir $out_dir unless -d $out_dir;

open(CSV, "<", "$db_partition/data_$dataset.csv") or die "cannot open dataset CSV file";
